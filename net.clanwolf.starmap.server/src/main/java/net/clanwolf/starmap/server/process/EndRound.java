...
public class EndRound {
	...
	private static boolean timeForThisRoundIsOver(Long seasonId) {
		...
		if (nextRoundDate != null && translatedNowDateWithTime != null) {
			...
			if (result) {
				logger.info("{} is before {}", dateTimeformatter.format(nextRoundDate), dateTimeformatter.format(translatedNowDateWithTime));
			} else {
				logger.info("{} is NOT before {}", dateTimeformatter.format(nextRoundDate), dateTimeformatter.format(translatedNowDateWithTime));
			}
			return result;
		} else {
			return false;
...
//		}
		...
		boolean attacksLeftToResolveInRound = !openAttacksInRoundList.isEmpty(); // We are ignoring the open attacks for the next round here
		...
		// -----------------------------------------------------------------------------------
		// Uncomment this only, if you need the round to be finalized anyway for local tests!
		// forceFinalize.set(true);
		// -----------------------------------------------------------------------------------

		logger.info("Checking if current round needs to be finalized.");
		if ((jumpshipsLeftToMove || attacksLeftToResolveInRound)
				&& !(timeForThisRoundIsOver(seasonId))
				&& !forceFinalize.get()) {
			// round is still active
			logger.info("Round is still active:");
			logger.info("--- {} jumpship(s) have moved.", jsmovedcount);
			logger.info("--- {} jumpship(s) have not moved.", jscount);
			logger.info("--- {} attacks still to be resolved.", openAttacksInRoundList.size());
			...
			try {
				...
				for (AttackPOJO attackPOJO : allAttacksForRound) {
					...
					logger.info("--- Attack: {} (StarSystem: {} / Authorized by {})", attackPOJO.getId(), attackPOJO.getStarSystemID(), rpCharacter);
...
//				}
				...
				for (AttackPOJO attackPOJO : allAttacksForRound) {
					...
					if(winnerId != null && !jpPojo.getJumpshipFactionID().equals(winnerId)){
						if(jpPojo.getStarSystemHistory().lastIndexOf(";") > 0) {
							String histo = jpPojo.getStarSystemHistory().substring(0, jpPojo.getStarSystemHistory().lastIndexOf(";"));
							jpPojo.setStarSystemHistory(histo);
					...
					logger.info("**** Storing winner for attack {} to be {}.", attackPOJO.getId(), winnerId);
		...
		return foughtAttacks.append("\r\n").append(resolvedAttacks).append("\r\n").append(movedJumpships).toString();
	}
