...
if (mysqli_num_rows($result) > 0) {
	while($row = mysqli_fetch_assoc($result)) {
		...
		if (is_null($user_birthdate)) {
			$date = new DateTime('1978-01-01');
			$user_birthdate = $date->format('Y-m-d');
			echo "Found null birthdate for user " . user_name . ". Setting to: " . $user_birthdate . "<br>";
		}
					   ...
					   echo "... processed!<br>";
						...
						if (mysqli_num_rows($result12) > 0) {
					   		while($row12 = mysqli_fetch_assoc($result12)) {
								// UPDATE Birthdate
								echo "Updating char to prevent null birthdate error.";
								...
								$sql18_update = $sql18_update . "WHERE ID = " . $last_user_id;
					   			...
					   			if(mysqli_query($conn, $sql18_update)) {
					   				echo "... processed!<br>";
					   			} else {
					   			   echo "... not processed! ERROR: Could not execute " . $sql18_update . ". Error: " . mysqli_error($conn) . "<br>";
					   			}
					   		}
						}
				   } else {
