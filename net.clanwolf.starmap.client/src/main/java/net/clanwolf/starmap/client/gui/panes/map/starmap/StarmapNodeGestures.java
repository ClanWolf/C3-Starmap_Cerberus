...
public class StarmapNodeGestures {
	private final static Logger logger = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

	private final StarmapDragContext nodeDragContext = new StarmapDragContext();
	private final EventHandler<MouseEvent> onStarSystemHoverEnteredEventHandler = event -> {
		if (StarmapConfig.MAP_HIGHLIGHT_HOVERED_STARSYSTEM) {
...
//			circleS2.setRadius(s2.getStarSystemCircle().getRadius());
	...
	StarmapNodeGestures(StarmapPannableCanvas canvas) {
		this.canvas = canvas;
	}
