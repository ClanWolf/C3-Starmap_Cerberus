...
public class SurfacemapPannableCanvas extends Pane {
	...
	SurfacemapPannableCanvas() {
		boUniverse = Nexus.getBoUniverse();
		setPrefSize(StarmapConfig.MAP_WIDTH, StarmapConfig.MAP_HEIGHT);
		...
		double radius2 = 30 * 2 * StarmapConfig.MAP_COORDINATES_MULTIPLICATOR; // 60 Lightyears
		starSystemMarkerCircle60ly = new Circle(radius2);
		starSystemMarkerCircle60ly.setStroke(StarmapConfig.MAP_RANGE_CIRCLE_COLOR.deriveColor(.5, .5, .5, 1.0));
		...
		double radius = 30 * StarmapConfig.MAP_COORDINATES_MULTIPLICATOR; // 30 Lightyears
		starSystemMarkerCircle30ly = new Circle(radius);
		starSystemMarkerCircle30ly.setStroke(StarmapConfig.MAP_RANGE_CIRCLE_COLOR.deriveColor(.5, .5, .5, 1.0));
		starSystemMarkerCircle30ly.setStrokeWidth(2);
		starSystemMarkerCircle30ly.getStrokeDashArray().setAll(10d, 10d, 10d, 10d);
		starSystemMarkerCircle30ly.setFill(StarmapConfig.MAP_RANGE_CIRCLE_COLOR.deriveColor(.6, .6, .6, 0.1));
	...
	public void addStarPane() {
		...
		for (int[] layer : StarmapConfig.BACKGROUND_STARS_LAYERS) {
			...
			for (int i = 0; i < number; i++) {
				...
				int size = (int) ((Math.random()) * StarmapConfig.BACKGROUND_STARS_MAX_SIZE + 1);
	...
	private void setGrid_500_Visible() {
		if (grid_500 != null) {
			if (myScale.get() >= StarmapConfig.zoomLevelToHideGrid1) {
				grid_500.setVisible(true);
				border.setVisible(true);
			} else if (myScale.get() < StarmapConfig.zoomLevelToHideGrid1) {
	...
	private void setGrid_250_Visible() {
		if (grid_250 != null) {
			if (myScale.get() >= StarmapConfig.zoomLevelToHideGrid2) {
				grid_250.setVisible(true);
				grid_center.setVisible(true);
			} else if (myScale.get() < StarmapConfig.zoomLevelToHideGrid2) {
	...
	private void setLevelIconsVisible() {
		if (boUniverse.starSystemBOs != null) {
			if (myScale.get() >= StarmapConfig.zoomLevelToHideLevelIcons) {
				for (BOStarSystem ss : boUniverse.starSystemBOs.values()) {
					Pane levelLabel = ss.getLevelLabel();
					levelLabel.setVisible(true);
				}
			} else if (myScale.get() < StarmapConfig.zoomLevelToHideLevelIcons) {
				for (BOStarSystem ss : boUniverse.starSystemBOs.values()) {
					Pane levelLabel = ss.getLevelLabel();
	...
	private void setStarSystemLabelsVisible() {
		if (boUniverse.starSystemBOs != null) {
			if (myScale.get() >= StarmapConfig.zoomLevelToHideStarSystemLabels) {
			...
			} else if (myScale.get() < StarmapConfig.zoomLevelToHideStarSystemLabels) {
				if (starSystemLabelsVisible) {
	...
	private void setStarSystemCirclesVisible() {
		if (boUniverse.starSystemBOs != null) {
			if (myScale.get() >= StarmapConfig.zoomLevelToHideStarSystemCircles) {
				for (BOStarSystem ss : boUniverse.starSystemBOs.values()) {
					Circle c = ss.getStarSystemCircle();
					c.setVisible(true);
				}
			} else if (myScale.get() < StarmapConfig.zoomLevelToHideStarSystemCircles) {
				for (BOStarSystem ss : boUniverse.starSystemBOs.values()) {
					Circle c = ss.getStarSystemCircle();
					c.setVisible(false);
				}
	...
	private void setAttacksVisible() {
		if (boUniverse.starSystemBOs != null && attacksPane != null) {
			if (myScale.get() >= StarmapConfig.zoomLevelToHideAttacks) {
				attacksPane.setVisible(true);
			} else if (myScale.get() < StarmapConfig.zoomLevelToHideAttacks) {
...
//						ship.getPredictedRouteLine().setVisible(false);
			...
			if (myScale.get() >= StarmapConfig.zoomLevelToHideJumpships) {
				for (BOJumpship ship : boUniverse.jumpshipBOs.values()) {
					if (ship.getJumpshipImageView() != null) {
...
//						ship.getPredictedRouteLine().setVisible(true);
			...
			} else if (myScale.get() < StarmapConfig.zoomLevelToHideJumpships) {
				for (BOJumpship ship : boUniverse.jumpshipBOs.values()) {
					if (ship.getJumpshipImageView() != null) {
						ship.getJumpshipImageView().setVisible(false);
//						ship.getPredictedRouteLine().setVisible(false);
