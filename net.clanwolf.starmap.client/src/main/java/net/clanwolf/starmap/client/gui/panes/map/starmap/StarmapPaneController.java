 ...
 * @author Meldric
 */
public class StarmapPaneController extends AbstractC3Controller implements ActionCallBackListener {
	...
	private StarmapPannableCanvas canvas;
	...
	private StarmapSceneGestures sceneGestures;
	private BOJumpship currentlyCenteredJumpship = null;
	private StarmapNodeGestures nodeGestures;
			...
			if (js.getJumpshipFaction() == Nexus.getCurrentUser().getCurrentCharacter().getFactionId()
				...
				if (s.getFactionId() != js.getJumpshipFaction() && createAttack) {
					...
					if (attackedSystem != null && attackerStartedFromSystem != null) {
						Platform.runLater(() -> {
							if (StarmapConfig.MAP_FLASH_ATTACKED_SYSTEMS) {
								PointD[] points = attackedSystem.getVoronoiRegion();
								if (points != null) {
									Circle circle = new Circle(attackedSystem.getScreenX(), attackedSystem.getScreenY(), StarmapConfig.MAP_BACKGROUND_AREA_RADIUS);
							...
							if (attackerStartedFromSystem != null) {
								if (StarmapConfig.MAP_FLASH_ATTACKED_SYSTEMS) {
									...
									if (points != null) {
										...
										double flashIntensity = StarmapConfig.MAP_FLASH_ATTACKED_SYSTEMS_INTENSITY;
										long flashDuration = StarmapConfig.MAP_FLASH_ATTACKED_SYSTEMS_DURATION;
										if (boAttack.getAttackDTO().getRound().equals(boUniverse.currentRound + 1)) {
											flashIntensity = StarmapConfig.MAP_FLASH_ATTACKED_SYSTEMS_INTENSITY_NEXTROUND;
											flashDuration = StarmapConfig.MAP_FLASH_ATTACKED_SYSTEMS_DURATION_NEXTROUND;
										}
...
//			mapButton05.setOpacity(0.0f);
			...
			try {
				canvas = new StarmapPannableCanvas();
				canvas.setTranslateX(StarmapConfig.MAP_INITIAL_TRANSLATE_X);
				canvas.setTranslateY(StarmapConfig.MAP_INITIAL_TRANSLATE_Y);
				canvas.setPrefSize(StarmapConfig.MAP_DIM, StarmapConfig.MAP_DIM);

				nodeGestures = new StarmapNodeGestures(canvas);
...
//			}
	...
	public void handleSurfaceMapButtonClick() {
		logger.info("Opening surface map...");

		surfaceMap = new SurfacemapPane(Nexus.getCurrentlySelectedStarSystem());
