...
class StarmapSceneGestures {
	private final static Logger logger = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

	private double previousX;
	private double previousY;

	private final StarmapDragContext sceneDragContext = new StarmapDragContext();
	private final StarmapPannableCanvas canvas;

	private boolean scrollingEnabled = true;

	StarmapSceneGestures(StarmapPannableCanvas canvas) {
		this.canvas = canvas;
	}
...
//		ActionManager.getAction(ACTIONS.UPDATE_COORD_INFO).execute("[" + String.format("%.2f", universeX) + ", " + String.format("%.2f", universeY) + "]");
	...
	private static double clamp(double value) {
		if (Double.compare(value, StarmapConfig.MAP_MIN_SCALE) < 0) {
			return StarmapConfig.MAP_MIN_SCALE;
		}
		if (Double.compare(value, StarmapConfig.MAP_MAX_SCALE) > 0) {
			return StarmapConfig.MAP_MAX_SCALE;
		}
		return value;
	}
